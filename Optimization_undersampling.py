...
def create_model(x_train_sequence, y_train, x_test_sequence, y_test):
    ...
    generator = BalancedBatchGenerator(x_train_sequence, y_train, sampler=NearMiss(), 
                                                batch_size=batch_size, random_state=1)
    
    result = model.fit_generator(generator=generator, epochs=num_epochs, verbose=verbose)
